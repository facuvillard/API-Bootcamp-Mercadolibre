...
import static com.mercadolibre.facundo_villard.util.ScopeUtils.SCOPE_VALUE;
import static com.mercadolibre.facundo_villard.util.ScopeUtils.isLocalScope;
...
public class SpringDocConfig {
	...
	public OpenAPI customOpenAPI(@Value("${app.title}") String appName, @Value("${app.description}") String description, @Value("${app.version}") String version) {
		OpenAPI api = new OpenAPI().info(new Info().title(appName)
				...
				.contact(new Contact().name("facundo_villard")
						.email("facundo.villard@mercadolibre.com")));
